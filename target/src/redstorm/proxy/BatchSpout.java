...
public class BatchSpout extends RubyObject implements IBatchSpout {
    ...
    static {
        String source = new StringBuilder("require 'java'\n" +
            ...
            "end\n" +
            "").toString();
        __ruby__.executeScript(source, "/Users/dev/.rvm/gems/jruby-1.7.12/gems/redstorm-0.6.6/lib/red_storm/proxy/batch_spout.rb");
    ...
    public void open(Map conf, TopologyContext context) {
        ...
        IRubyObject ruby_result = Helpers.invoke(__ruby__.getCurrentContext(), this, "open", ruby_arg_conf, ruby_arg_context);
        return;
    ...
    public void emitBatch(long batch_id, TridentCollector collector) {
        IRubyObject ruby_arg_batch_id = JavaUtil.convertJavaToRuby(__ruby__, batch_id);
        IRubyObject ruby_arg_collector = JavaUtil.convertJavaToRuby(__ruby__, collector);
        IRubyObject ruby_result = Helpers.invoke(__ruby__.getCurrentContext(), this, "emitBatch", ruby_arg_batch_id, ruby_arg_collector);
        return;
    ...
    public void ack(long batch_id) {
        IRubyObject ruby_arg_batch_id = JavaUtil.convertJavaToRuby(__ruby__, batch_id);
        IRubyObject ruby_result = Helpers.invoke(__ruby__.getCurrentContext(), this, "ack", ruby_arg_batch_id);
        return;
