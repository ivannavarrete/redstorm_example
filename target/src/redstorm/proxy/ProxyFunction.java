...
public class ProxyFunction extends RubyObject implements Function {
    ...
    static {
        String source = new StringBuilder("require 'java'\n" +
            ...
            "end\n" +
            "").toString();
        __ruby__.executeScript(source, "/Users/dev/.rvm/gems/jruby-1.7.12/gems/redstorm-0.6.6/lib/red_storm/proxy/proxy_function.rb");
    ...
    public  ProxyFunction(String base_class_path, String real_class_name) {
        ...
        IRubyObject ruby_arg_real_class_name = JavaUtil.convertJavaToRuby(__ruby__, real_class_name);
        Helpers.invoke(__ruby__.getCurrentContext(), this, "initialize", ruby_arg_base_class_path, ruby_arg_real_class_name);

    }

    
    public void execute(TridentTuple _trident_tuple, TridentCollector _trident_collector) {
        IRubyObject ruby_arg__trident_tuple = JavaUtil.convertJavaToRuby(__ruby__, _trident_tuple);
        IRubyObject ruby_arg__trident_collector = JavaUtil.convertJavaToRuby(__ruby__, _trident_collector);
        IRubyObject ruby_result = Helpers.invoke(__ruby__.getCurrentContext(), this, "execute", ruby_arg__trident_tuple, ruby_arg__trident_collector);
        return;
    ...
    public void prepare(Map _map, TridentOperationContext _trident_operation_context) {
        IRubyObject ruby_arg__map = JavaUtil.convertJavaToRuby(__ruby__, _map);
        IRubyObject ruby_arg__trident_operation_context = JavaUtil.convertJavaToRuby(__ruby__, _trident_operation_context);
        IRubyObject ruby_result = Helpers.invoke(__ruby__.getCurrentContext(), this, "prepare", ruby_arg__map, ruby_arg__trident_operation_context);
        return;
